#ifndef SOSSO_REACTOR_HPP
#define SOSSO_REACTOR_HPP
...
public:
  ...
  bool read_probes(unsigned period, unsigned repetitions) {
    if (!_in.recording()) {
    ...
    Log::info(SOSSO_LOC, "Step of %u is %lld ns.", _in.stepping(),
              _in.frames_to_time(_in.stepping()));
    // Initialize correction parameters.
    _in_correction.set_drift_limits(-64, 64);
    _out_correction.set_drift_limits(-64, 64);
